...
class ManualSortApp:
    ...
    def init_vars(self): # initialize some member variables
        ...
        self.sortDict = dict.fromkeys(self.picturesList)
        
        img = ImageTk.PhotoImage(Image.open(self.picturesList[self.currImageIndex]).resize((500,500)))
    ...
    def updatePic(self):
        # print(picturesList)
        current_img  = ImageTk.PhotoImage(Image.open(self.picturesList[self.currImageIndex]).resize((500,500)))
    ...
    def init_navigation(self):
        ...
        self.backButton = tk.Button(text='<',borderwidth=1, relief="solid", command=self.moveBackward, font=('Helvetica',20))
        self.backButton.grid(row=1,column=0)
        
        # forwardImg = ImageTk.PhotoImage(Image.open("Screenshots/forward.ico"))
        self.forwardButton = tk.Button(text='>', borderwidth=1, relief="solid", command=self.moveForward, font=('Helvetica',20))
    ...
    def init_sort_buttons(self):
        ...
        keepButton = tk.Button(f1, text='Keep', width=w, height=h, command=self.keepPic, font=('Helvetica',15))
        # keepButton.grid(row=1,column=2)
        # formerly relief=tk.FLAT
        
        discardButton = tk.Button(f1, text='Discard', borderwidth=1, command=self.discardPic, font=('Helvetica',15))
        # discardButton.grid(row=1,column=3)
        
        maybeButton = tk.Button(f1, text='Maybe', borderwidth=1, command=self.maybePic, font=('Helvetica',15))
        # maybeButton.grid(row=1,column=4)
        
        keepButton.grid(row=0,column=0) # pack(side="left", fill="both", expand=True) # side="left"
        discardButton.grid(row=0,column=1) # pack(fill="x") # side="middle"
        maybeButton.grid(row=0,column=2) # pack(side="right", fill="both", expand=True)
